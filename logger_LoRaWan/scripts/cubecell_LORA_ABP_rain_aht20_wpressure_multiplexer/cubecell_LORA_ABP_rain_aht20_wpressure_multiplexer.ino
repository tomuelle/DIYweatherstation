...
uint32_t temperature_water, pressure_water;
uint32_t pressure;
...
static void prepareTxFrame( uint8_t port )
{
  ...
  for (int i = 1; i <= nsize; i++) {
    ret = Dps310PressureSensor.measurePressureOnce(Detection_array[i], 7);
    if (Detection_array[i] - Detection_array[i - 1] < 5)
    {
      state1 += Detection_array[i];
      Serial.println(Detection_array[i]);
      nDiv += 1;
    }
  }
  state1 = state1 / nDiv;
  ...
  appData[8] = highByte(temperature_water);
  appData[9] = lowByte(temperature_water);

  appData[10] = (byte) ((pressure_water & 0xFF000000) >> 24 );
  appData[11] = (byte) ((pressure_water & 0x00FF0000) >> 16 );
  appData[12] = (byte) ((pressure_water & 0x0000FF00) >> 8  );
  appData[13] = (byte) ((pressure_water & 0X000000FF)       );

  appData[14] = highByte(rain);
  appData[15] = lowByte(rain);

  appData[16] = highByte(cumulRain);
  appData[17] = lowByte(cumulRain);

  appData[18] = highByte(batteryVoltage);
  appData[19] = lowByte(batteryVoltage);
  ...
  if (ENABLE_SERIAL) {
    ...
    Serial.print(" mbar, Water Temperature: ");
    Serial.print(temperature_water / 100);
    Serial.print("C, Water Pressure: ");
    Serial.print(pressure_water / 100);
