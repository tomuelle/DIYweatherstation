...
const char *DevEUI = "add your key";
const char *AppEUI = "add your key";
const char *AppKey = "add your key";
char rgbTxt[24];
...
void takeAction()
{
  ...
  if (rainEvent == 1  && (int(interruptTime.get() - myTime.get()) > 0)) {
    ...
    if (isnan(humidity) || humidity > 10000 || humidity <= 0 || temperature > 9900) {
      humidity = 99900;
      temperature = 99900;
      pressure = 99900;
    ...
    if (checkSD == false) {
      interruptTime = DateTime(myTime.get()  + int(((((floor(myTime.minute() / (interruptInterval / 60)) + 1) * (interruptInterval / 60)) - myTime.minute()) * 60) - myTime.second()));
      alarmLED();
      debugSerial.print(F("Fail to save to SD card"));
      //systemSleep();
      return;
    }
    else {
      debugSerial.print(F("Data saved to SD card : "));
    }
...
void setupLoRa() {
  ...
  checkBeeCommand("ok", 2000);
  loraSerial.println("mac set devaddr 00000000");
  checkBeeCommand("ok", 2000);
  loraSerial.println("mac set nwkskey 00000000000000000000000000000000");
  checkBeeCommand("ok", 2000);
  loraSerial.println("mac set appskey 00000000000000000000000000000000");
  checkBeeCommand("ok", 2000);
...
void sendLoRa() {
  ...
  int i = 3;
  while (!checkBeeCommand("mac_tx_ok", 5000) && (i > 0) ) {
    setupLoRa();
    loraSerial.print(F("mac tx uncnf 1 "));
    loraSerial.println(rgbTxt);
    alarmLED();
...
boolean checkBeeCommand(String StrCheck, uint16_t timeint) {
  ...
  while (1) {
    ...
    if ((message.indexOf("not_joined") >= 0) or (message.indexOf("denied") >= 0)) {
      check = false;
